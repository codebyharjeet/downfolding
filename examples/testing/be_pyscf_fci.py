...
constant, one_body, two_body, three_body = ducc.get_ducc_ham_active_space_pyscf(fmat,vmat,t1_amps,t2_amps,n_a,n_b,n_orb,n_occ,n_act,inc_3_body=False)
...
if bare_ducc == "bare":
    ...
    fmat_spatial = ducc.get_spin_to_spatial_1(fmat,n_act)
    vmat_spatial = ducc.get_spin_to_spatial_1(vmat,n_act)

    constant = 0

if bare_ducc == "ducc":
    umat_1 = ducc.get_u(two_body,range(n_a),range(n_b))
    one_body -= umat_1

    fmat_spatial = ducc.get_spin_to_spatial_1(one_body,n_act)
    vmat_spatial = ducc.get_spin_to_spatial_1(two_body,n_act)
    #constant = 0
...
print(" E_corr:   %12.8f"%(efci_orgnl-E_scf))
