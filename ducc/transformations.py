        ...
        for q in range(0, n_occ):
            for r in range(0, n_occ):
                for s in range(0, n_occ):
                    # OO|OO
                    two_body_op += of.FermionOperator(
                        ((s, 0), (r, 0), (p, 1), (q, 1)), two_body_tens[p, q, r, s]
                    )
    for p in range(0, n_occ):
        for q in range(0, n_occ):
            for r in range(0, n_occ):
                for s in range(n_occ, 2 * n_orb):
                    # OO|OV
                    two_body_op += of.FermionOperator(
                        ((r, 0), (s, 0), (p, 1), (q, 1)), -two_body_tens[p, q, r, s]
                    )
                    # OO|VO
                    two_body_op += of.FermionOperator(
                        ((r, 0), (s, 0), (p, 1), (q, 1)), two_body_tens[p, q, s, r]
                    )
                    # OV|OO
                    two_body_op += of.FermionOperator(
                        ((s, 1), (q, 0), (r, 0), (p, 1)), -two_body_tens[p, s, r, q]
                    )
                    # VO|OO
                    two_body_op += of.FermionOperator(
                        ((s, 1), (p, 0), (r, 0), (q, 1)), two_body_tens[s, q, r, p]
                    )
    for p in range(0, n_occ):
        for q in range(0, n_occ):
            for r in range(n_occ, 2 * n_orb):
                for s in range(n_occ, 2 * n_orb):
                    ...
                    # OV|OV
                    two_body_op += of.FermionOperator(
                        ((r, 1), (q, 0), (s, 0), (p, 1)), two_body_tens[p, r, q, s]
                    )
                    # VO|OV
                    two_body_op += of.FermionOperator(
                        ((r, 1), (p, 0), (s, 0), (q, 1)), -two_body_tens[r, q, p, s]
                    )
                    # OV|VO
                    two_body_op += of.FermionOperator(
                        ((s, 1), (q, 0), (r, 0), (p, 1)), -two_body_tens[p, s, r, q]
                    ...
                    # VV|OO
                    two_body_op += of.FermionOperator(
                        ((r, 1), (s, 1), (q, 0), (p, 0)), two_body_tens[r, s, p, q]
                    )
    for p in range(0, n_occ):
        for q in range(n_occ, 2 * n_orb):
            for r in range(n_occ, 2 * n_orb):
                for s in range(n_occ, 2 * n_orb):
                    # OV|VV
                    two_body_op += of.FermionOperator(
                        ((q, 1), (s, 0), (r, 0), (p, 1)), -two_body_tens[p, q, r, s]
                    )
                    # VO|VV
                    two_body_op += of.FermionOperator(
                        ((q, 1), (s, 0), (r, 0), (p, 1)), two_body_tens[q, p, r, s]
                    )
                    # VV|OV
                    two_body_op += of.FermionOperator(
                        ((r, 1), (q, 1), (p, 0), (s, 0)), -two_body_tens[r, q, p, s]
                    )
                    # VV|VO
                    two_body_op += of.FermionOperator(
                        ((s, 1), (q, 1), (p, 0), (r, 0)), two_body_tens[s, q, r, p]
                    )
    for p in range(n_occ, 2 * n_orb):
        for q in range(n_occ, 2 * n_orb):
            for r in range(n_occ, 2 * n_orb):
                for s in range(n_occ, 2 * n_orb):
                    # VVVV
                    two_body_op += of.FermionOperator(
                        ((p, 1), (q, 1), (s, 0), (r, 0)), two_body_tens[p, q, r, s]
                    )
    return two_body_op
                ...
                for s in range(0, n_occ):
                    for t in range(0, n_occ):
                        for u in range(0, n_occ):
                            # OOO|OOO
                            three_body_op += of.FermionOperator(
                                ((u, 0), (t, 0), (s, 0), (p, 1), (q, 1), (r, 1)),
                                -three_body_tens[p, q, r, s, t, u],
                            )
    for p in range(0, n_occ):
        for q in range(0, n_occ):
            for r in range(0, n_occ):
                for s in range(0, n_occ):
                    for t in range(0, n_occ):
                        for u in range(n_occ, 2 * n_orb):
                            # OOO|OOV
                            three_body_op += of.FermionOperator(
                                ((t, 0), (s, 0), (u, 0), (p, 1), (q, 1), (r, 1)),
                                -three_body_tens[p, q, r, s, t, u],
                            )
                            # OOO|OVO
                            three_body_op += of.FermionOperator(
                                ((t, 0), (s, 0), (u, 0), (p, 1), (q, 1), (r, 1)),
                                three_body_tens[p, q, r, s, u, t],
                            )
                            # OOO|VOO
                            three_body_op += of.FermionOperator(
                                ((s, 0), (t, 0), (u, 0), (p, 1), (q, 1), (r, 1)),
                                -three_body_tens[p, q, r, u, t, s],
                            )
                            # OOV|OOO
                            three_body_op += of.FermionOperator(
                                ((u, 1), (r, 0), (t, 0), (s, 0), (p, 1), (q, 1)),
                                three_body_tens[p, q, u, s, t, r],
                            )
                            # OVO|OOO
                            three_body_op += of.FermionOperator(
                                ((u, 1), (q, 0), (t, 0), (s, 0), (p, 1), (r, 1)),
                                -three_body_tens[p, u, r, s, t, q],
                            )
                            # VOO|OOO
                            three_body_op += of.FermionOperator(
                                ((u, 1), (p, 0), (t, 0), (s, 0), (q, 1), (r, 1)),
                                three_body_tens[u, q, r, s, t, p],
                            )
    for p in range(0, n_occ):
        for q in range(0, n_occ):
            for r in range(0, n_occ):
                for s in range(0, n_occ):
                    for t in range(n_occ, 2 * n_orb):
                        for u in range(n_occ, 2 * n_orb):
                            # OOO|OVV
                            three_body_op += of.FermionOperator(
                                ((s, 0), (u, 0), (t, 0), (p, 1), (q, 1), (r, 1)),
                                -three_body_tens[p, q, r, s, t, u],
                            )
                            # OOO|VOV
                            three_body_op += of.FermionOperator(
                                ((s, 0), (u, 0), (t, 0), (p, 1), (q, 1), (r, 1)),
                                three_body_tens[p, q, r, t, s, u],
                            )
                            # OOV|OOV
                            three_body_op += of.FermionOperator(
                                ((t, 1), (r, 0), (s, 0), (u, 0), (p, 1), (q, 1)),
                                three_body_tens[p, q, t, s, r, u],
                            )
                            # OVO|OOV
                            three_body_op += of.FermionOperator(
                                ((t, 1), (q, 0), (s, 0), (u, 0), (p, 1), (r, 1)),
                                -three_body_tens[p, t, r, s, q, u],
                            )
                            # VOO|OOV
                            three_body_op += of.FermionOperator(
                                ((t, 1), (p, 0), (s, 0), (u, 0), (q, 1), (r, 1)),
                                three_body_tens[t, q, r, s, p, u],
                            )
                            # OOO|VVO
                            three_body_op += of.FermionOperator(
                                ((s, 0), (t, 0), (u, 0), (p, 1), (q, 1), (r, 1)),
                            ...
                            # OOV|OVO
                            three_body_op += of.FermionOperator(
                                ((u, 1), (r, 0), (s, 0), (t, 0), (p, 1), (q, 1)),
                                -three_body_tens[p, q, u, s, t, r],
                            )
                            # OVO|OVO
                            three_body_op += of.FermionOperator(
                                ((u, 1), (q, 0), (s, 0), (t, 0), (p, 1), (r, 1)),
                                three_body_tens[p, u, r, s, t, q],
                            )
                            # VOO|OVO
                            three_body_op += of.FermionOperator(
                                ((u, 1), (p, 0), (s, 0), (t, 0), (q, 1), (r, 1)),
                                -three_body_tens[u, q, r, s, t, p],
                            )
                            # OOV|VOO
                            three_body_op += of.FermionOperator(
                                ((t, 1), (s, 0), (r, 0), (u, 0), (p, 1), (q, 1)),
                                three_body_tens[p, q, t, u, r, s],
                            )
                            # OVO|VOO
                            three_body_op += of.FermionOperator(
                                ((t, 1), (s, 0), (q, 0), (u, 0), (p, 1), (r, 1)),
                                -three_body_tens[p, t, r, u, q, s],
                            )
                            # VOO|VOO
                            three_body_op += of.FermionOperator(
                                ((t, 1), (s, 0), (p, 0), (u, 0), (q, 1), (r, 1)),
                                three_body_tens[t, q, r, u, p, s],
                            )
                            # OVV|OOO
                            three_body_op += of.FermionOperator(
                                ((t, 1), (u, 1), (r, 0), (q, 0), (s, 0), (p, 1)),
                                -three_body_tens[p, t, u, s, q, r],
                            )
                            # VOV|OOO
                            three_body_op += of.FermionOperator(
                                ((t, 1), (u, 1), (r, 0), (p, 0), (s, 0), (q, 1)),
                                three_body_tens[t, q, u, s, p, r],
                            )
                            # VVO|OOO
                            three_body_op += of.FermionOperator(
                                ((t, 1), (u, 1), (q, 0), (p, 0), (s, 0), (r, 1)),
                                -three_body_tens[t, u, r, s, p, q],
                            )
    for p in range(0, n_occ):
        for q in range(0, n_occ):
            for r in range(0, n_occ):
                for s in range(n_occ, 2 * n_orb):
                    for t in range(n_occ, 2 * n_orb):
                        for u in range(n_occ, 2 * n_orb):
                            ...
                            # OOV|OVV
                            three_body_op += of.FermionOperator(
                                ((s, 1), (r, 0), (u, 0), (t, 0), (p, 1), (q, 1)),
                                three_body_tens[p, q, s, r, t, u],
                            )
                            # OVO|OVV
                            three_body_op += of.FermionOperator(
                                ((s, 1), (q, 0), (u, 0), (t, 0), (p, 1), (r, 1)),
                                -three_body_tens[p, s, r, q, t, u],
                            )
                            # VOO|OVV
                            three_body_op += of.FermionOperator(
                                ((s, 1), (p, 0), (u, 0), (t, 0), (q, 1), (r, 1)),
                                three_body_tens[s, q, r, p, t, u],
                            )
                            # OOV|VOV
                            three_body_op += of.FermionOperator(
                                ((t, 1), (r, 0), (u, 0), (s, 0), (p, 1), (q, 1)),
                                -three_body_tens[p, q, t, s, r, u],
                            )
                            # OVO|VOV
                            three_body_op += of.FermionOperator(
                                ((t, 1), (q, 0), (u, 0), (s, 0), (p, 1), (r, 1)),
                                three_body_tens[p, t, r, s, q, u],
                            )
                            # VOO|VOV
                            three_body_op += of.FermionOperator(
                                ((t, 1), (p, 0), (u, 0), (s, 0), (q, 1), (r, 1)),
                                -three_body_tens[t, q, r, s, p, u],
                            )
                            # OOV|VVO
                            three_body_op += of.FermionOperator(
                            ...
                            # OVV|OOV
                            three_body_op += of.FermionOperator(
                                ((s, 1), (t, 1), (r, 0), (q, 0), (u, 0), (p, 1)),
                                -three_body_tens[p, s, t, q, r, u],
                            )
                            # VOV|OOV
                            three_body_op += of.FermionOperator(
                                ((s, 1), (t, 1), (r, 0), (p, 0), (u, 0), (q, 1)),
                                three_body_tens[s, q, t, p, r, u],
                            )
                            # VVO|OOV
                            three_body_op += of.FermionOperator(
                                ((s, 1), (t, 1), (q, 0), (p, 0), (u, 0), (r, 1)),
                                -three_body_tens[s, t, r, p, q, u],
                            )
                            # OVV|OVO
                            three_body_op += of.FermionOperator(
                                ((s, 1), (u, 1), (r, 0), (q, 0), (t, 0), (p, 1)),
                                three_body_tens[p, s, u, q, t, r],
                            )
                            # VOV|OVO
                            three_body_op += of.FermionOperator(
                                ((s, 1), (u, 1), (r, 0), (p, 0), (t, 0), (q, 1)),
                                -three_body_tens[s, q, u, p, t, r],
                            )
                            # VVO|OVO
                            three_body_op += of.FermionOperator(
                                ((s, 1), (u, 1), (q, 0), (p, 0), (t, 0), (r, 1)),
                                three_body_tens[s, u, r, p, t, q],
                            )
                            # OVV|VOO
                            three_body_op += of.FermionOperator(
                                ((t, 1), (u, 1), (r, 0), (q, 0), (s, 0), (p, 1)),
                            ...
                            # VVV|OOO
                            three_body_op += of.FermionOperator(
                                ((s, 1), (t, 1), (u, 1), (r, 0), (q, 0), (p, 0)),
                                three_body_tens[s, t, u, p, q, r],
                            )
    for p in range(0, n_occ):
        for q in range(0, n_occ):
            for r in range(n_occ, 2 * n_orb):
                for s in range(n_occ, 2 * n_orb):
                    for t in range(n_occ, 2 * n_orb):
                        for u in range(n_occ, 2 * n_orb):
                            # OOV|VVV
                            three_body_op += of.FermionOperator(
                                ((r, 1), (u, 0), (t, 0), (s, 0), (p, 1), (q, 1)),
                                three_body_tens[p, q, r, s, t, u],
                            )
                            # OVO|VVV
                            three_body_op += of.FermionOperator(
                                ((r, 1), (u, 0), (t, 0), (s, 0), (p, 1), (q, 1)),
                                -three_body_tens[p, r, q, s, t, u],
                            )
                            # VOO|VVV
                            three_body_op += of.FermionOperator(
                                ((r, 1), (u, 0), (t, 0), (s, 0), (q, 1), (p, 1)),
                                three_body_tens[r, q, p, s, t, u],
                            )
                            # OVV|OVV
                            three_body_op += of.FermionOperator(
                                ((s, 1), (r, 1), (q, 0), (u, 0), (t, 0), (p, 1)),
                                -three_body_tens[p, s, r, q, t, u],
                            )
                            # VOV|OVV
                            three_body_op += of.FermionOperator(
                                ((s, 1), (r, 1), (p, 0), (u, 0), (t, 0), (q, 1)),
                                three_body_tens[s, q, r, p, t, u],
                            )
                            # VVO|OVV
                            three_body_op += of.FermionOperator(
                                ((s, 1), (r, 1), (p, 0), (u, 0), (t, 0), (q, 1)),
                                -three_body_tens[s, r, q, p, t, u],
                            )
                            # OVV|VOV
                            three_body_op += of.FermionOperator(
                                ((t, 1), (r, 1), (q, 0), (u, 0), (s, 0), (p, 1)),
                                three_body_tens[p, t, r, s, q, u],
                            )
                            # VOV|VOV
                            three_body_op += of.FermionOperator(
                                ((t, 1), (r, 1), (p, 0), (u, 0), (s, 0), (q, 1)),
                                -three_body_tens[t, q, r, s, p, u],
                            )
                            # VVO|VOV
                            three_body_op += of.FermionOperator(
                                ((t, 1), (r, 1), (p, 0), (u, 0), (s, 0), (q, 1)),
                                three_body_tens[t, r, q, s, p, u],
                            )
                            # OVV|VVO
                            three_body_op += of.FermionOperator(
                                ((u, 1), (r, 1), (q, 0), (t, 0), (s, 0), (p, 1)),
                                -three_body_tens[p, u, r, s, t, q],
                            )
                            # VOV|VVO
                            three_body_op += of.FermionOperator(
                                ((u, 1), (r, 1), (p, 0), (t, 0), (s, 0), (q, 1)),
                                three_body_tens[u, q, r, s, t, p],
                            )
                            # VVO|VVO
                            three_body_op += of.FermionOperator(
                                ((u, 1), (r, 1), (p, 0), (t, 0), (s, 0), (q, 1)),
                                -three_body_tens[u, r, q, s, t, p],
                            )
                            # VVV|OOV
                            three_body_op += of.FermionOperator(
                                ((s, 1), (t, 1), (r, 1), (q, 0), (p, 0), (u, 0)),
                                three_body_tens[s, t, r, p, q, u],
                            )
                            # VVV|OVO
                            three_body_op += of.FermionOperator(
                                ((s, 1), (u, 1), (r, 1), (q, 0), (p, 0), (t, 0)),
                                -three_body_tens[s, u, r, p, t, q],
                            )
                            # VVV|VOO
                            three_body_op += of.FermionOperator(
                                ((t, 1), (u, 1), (r, 1), (q, 0), (p, 0), (s, 0)),
                                three_body_tens[t, u, r, s, p, q],
                            )
    for p in range(0, n_occ):
        for q in range(n_occ, 2 * n_orb):
                ...
                for s in range(n_occ, 2 * n_orb):
                    for t in range(n_occ, 2 * n_orb):
                        for u in range(n_occ, 2 * n_orb):
                            # OVV|VVV
                            three_body_op += of.FermionOperator(
                                ((q, 1), (r, 1), (u, 0), (t, 0), (s, 0), (p, 1)),
                                -three_body_tens[p, q, r, s, t, u],
                            )
                            # VOV|VVV
                            three_body_op += of.FermionOperator(
                                ((q, 1), (r, 1), (u, 0), (t, 0), (s, 0), (p, 1)),
                                three_body_tens[q, p, r, s, t, u],
                            )
                            # VVO|VVV
                            three_body_op += of.FermionOperator(
                                ((r, 1), (q, 1), (u, 0), (t, 0), (s, 0), (p, 1)),
                                -three_body_tens[r, q, p, s, t, u],
                            )
                            # VVV|OVV
                            three_body_op += of.FermionOperator(
                                ((s, 1), (q, 1), (r, 1), (p, 0), (u, 0), (t, 0)),
                                three_body_tens[s, q, r, p, t, u],
                            )
                            # VVV|VOV
                            three_body_op += of.FermionOperator(
                                ((t, 1), (q, 1), (r, 1), (p, 0), (u, 0), (s, 0)),
                                -three_body_tens[t, q, r, s, p, u],
                            )
                            # VVV|VVO
                            three_body_op += of.FermionOperator(
                                ((u, 1), (q, 1), (r, 1), (p, 0), (t, 0), (s, 0)),
                                three_body_tens[u, q, r, s, t, p],
                            )
    for p in range(n_occ, 2 * n_orb):
        for q in range(n_occ, 2 * n_orb):
